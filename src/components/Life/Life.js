  ...
  return (
    <ImageListItem key={alt} sx={{ overflow: "visible" }} ref={itemRef}>
      <motion.div
        ...
        style={{ y, opacity, borderRadius: "1.6rem", overflow: "hidden" }}
      >
        <a href={href || src}>
          <LazyImage
            src={src}
...
export default function Life(props) {
  ...
  return (
    <Fragment>
          ...
          <Tab key="san" title="a blend w/ my friends :)">
            <BrowserTabBox>
              <iframe
                ...
                src="https://open.spotify.com/embed/playlist/37i9dQZF1EJJf1yNS86eOl?utm_source=generator"
                width="100%"
                frameBorder={0}
                allowFullScreen=""
                allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"
      ...
      >
        just started experimenting with my first camera: a $200 fuji x-t1 and a
        $80 lens!
      </Typography>
      <ImageList
        variant="masonry"
        cols={smbk ? 2 : 1}
        gap={"max(3.2rem, 6.4vw)"}
        sx={{
          marginTop: 8,
          marginBottom: 5,
          overflow: "visible",
