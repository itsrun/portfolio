  ...
  return (
    <motion.div style={{ y, opacity, scale }}>
      <ItemBox ref={targetRef}>
        <ProjectImg images={images} idx={idx} />
        <TextBox>
          <Link
            color="secondary"
            sx={{
          ...
          {award && (
            <Typography
              ...
              href={award.link}
            >
              <EmojiEventsIcon sx={{ marginRight: 0.5 }} />
              {award.name}
            </Typography>
          )}
          {place && (
...
export default function Project(props) {
  return (
    <Stack spacing={8} id="projects">
      ...
      {data.map((props, idx) => (
        <ProjectItem key={idx} idx={idx} {...props} />
      ))}
    </Stack>
  );
}
